...
async function initRemoteStorage(api, siteKey, storage) {
  try {
    const promiseFactory = () => fetch(`${api}/vstrg/v1/${siteKey}`).then(
        async (r) => {
          if (!r.ok) {
...
async function initTelemetry(api, clientKey) {
  try {
    const promiseFactory = () => fetch(`${api}/analytics/v1/${clientKey}`).then(
        async (r) => {
          if (!r.ok) {
            const message = await r.text();
            throw createError(message);
...
async function loadRunnerCode(api, clientKey, vmId) {
  try {
    const promiseFactory = () => fetch(`${api}/challenges/v1/${clientKey}/${vmId}`).then(
        async (r) => {
          if (!r.ok) {
            const message = await r.text();
            throw createError(message);
...
const factory = () => {
  ...
  const getRunner = async (api, clientKey) => {
    let runner;
    try {
      const { code, timestamp } = await loadRunnerCode(api, clientKey, vmId);
